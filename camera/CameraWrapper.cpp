...
#define LOG_TAG "KLTECameraWrapper"
                ...
                hw_device_t** device)
{
    ...
    if (name != NULL) {
        ...
        if (!fixed_set_params) {
            ALOGE("%s(%d): parameter memory allocation fail", __FUNCTION__, __LINE__);
            rv = -ENOMEM;
            goto fail;
        }
        else
            ALOGI("%s(%d): parameter memory allocation success", __FUNCTION__, __LINE__);
        memset(fixed_set_params, 0, sizeof(char *) * num_cameras);

        if(cameraid > num_cameras)
        {
            ALOGE("%s(%d): camera service provided cameraid out of bounds, "
                    "cameraid = %d, num supported = %d", __FUNCTION__, __LINE__,
                    cameraid, num_cameras);
            rv = -EINVAL;
            goto fail;
        }
        else
            ALOGI("%s(%d): camera service provided camerid = %d is IN BOUNDS", __FUNCTION__, __LINE__,cameraid);

        camera_device = (wrapper_camera_device_t*)malloc(sizeof(*camera_device));
        if(!camera_device)
        {
            ALOGE("%s(%d): camera_device allocation fail", __FUNCTION__, __LINE__);
            rv = -ENOMEM;
            goto fail;
        }
        else
        ...
        if (rv) {
            ALOGE("%s(%d): vendor camera open fail", __FUNCTION__, __LINE__);
            goto fail;
        }
        ALOGI("%s(%d): got vendor camera device 0x%08X", __FUNCTION__, __LINE__, (uintptr_t)(camera_device->vendor));
        ...
        ALOGI("%s(%d): got vendor camera_ops allocation success", __FUNCTION__, __LINE__);
        ...
        ALOGI("%s(%d): callbacks registered", __FUNCTION__, __LINE__);
